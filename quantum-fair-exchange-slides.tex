...
\begin{frame}{Feasibility: What We Show}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.61\textwidth}
      ...
      \uncover<2->{
        \vspace{0.85em}
        {\large\bfseries Input-state preservation (soundness)\par}
        \vspace{0.15em}
...
\begin{frame}{Background: VQFHE}
  ...
  \begin{tikzpicture}[
    ...
    \draw[qmsg] (3.1,-0.52) -- (-3.1,-0.52)
      ...
      {$\ket{\widetilde\psi}$};
    \node[anchor=east,align=right,font=\small] at (-3.35,-1.02)
      {$\mathsf{VerDec}_{\mathsf{sk}}(\ket{\widetilde\psi})$\\[-0.1em]
  ...
  \note{
    ...
    \begin{itemize}
      ...
      \item This is a deliberately simplified public-key interface, separated from the concrete CNOT mechanism on the next slide.
      \item In the full VQFHE syntax, an evaluation key and a computation log may appear separately. Here the evaluation key is folded into $\mathsf{pk}$, and the log into the returned $\ket{\widetilde\psi}$.
...
\begin{frame}{Background: Homomorphic CNOT}
  \centering
  {\small Both ciphertexts use the \textbf{same hidden permutation} $\pi$
    and \textbf{independent one-time pads}.\par}
...
\begin{frame}{Background: Homomorphic CNOT Analysis}
  ...
  \note{
    \footnotesize
    \begin{itemize}
      \item This slide is the analysis of the preceding construction. Read it from top to bottom in the same order as $Q\pi(\cdot)$: pad, permutation, data, tests.
...
\begin{frame}{Construction I: Teleport to the TTP}
  ...
  \begin{tikzpicture}[
    ...
    \draw[qmsg] (3.05,0.42) -- (-3.05,0.42)
      node[midway,above=2pt,font=\small,fill=white,inner sep=1pt]
      {one half};
    \node[font=\large] at (-3.35,-0.18) {$\qasset{A}$};
    \draw[cmsg] (-3.05,-0.72) -- (3.05,-0.72)
...
\newcommand{\checkedgatediagram}[5]{%
  \begin{tikzpicture}[
    ...
    \draw[cmsg] (-4.75,-1.84) -- (0.25,-1.84)
      ...
      {$o_0\leftarrow\mathsf{Meas}_Z((G^\dagger)^{\otimes2}\ket{T_0})$\\[-0.1em]
       $o_+\leftarrow\mathsf{Meas}_X((G^\dagger)^{\otimes2}\ket{T_+})$};
    \node[font=\scriptsize,align=center] at (0.25,-2.27) {#5};
...
\begin{frame}{Construction II: One Checked Gate}
  \centering
  {\scriptsize
    $G:=g^{(j)}$,\quad
    $\ket W:=\pi_E\bigl(\ket{D_j},X^e\ket{0^w},Z^f\ket{+^w}\bigr)$,
    \\[-0.05em]
    {\vphantom{$\pi_E^{-1}R\pi_E
      =R_D\otimes R_0\otimes R_+,\quad
      G^\dagger R_sG=X^{a_s}Z^{b_s}$}%
      \phantom{without re-encryption.}}
    \par}
  ...
  \checkedgatediagram
    ...
    {check the trap outcomes $o_0,o_+$;\\[-0.1em]
      keep $G\ket{D_j}$}
  ...
  \note{
    ...
    \begin{itemize}
      \setlength{\itemsep}{0pt}
      \item This is the warm-up version with no fresh re-encryption. It isolates the four-message check; the next frame adds the missing security layer without moving anything.
      \item This is one complete per-coordinate interaction. The TTP checks Alice's reported outcomes before advancing to $j+1$; Bob never touches all coordinates before a check occurs.
      ...
      \item Here $e,f,u,v\in\{0,1\}^w$ and $\pi_0,\pi_+\in S_2$ are fresh. Alice receives each returned test mixed with a fresh test of a known gate output.
      ...
      \item In an honest execution, undoing $G$ makes the old and fresh traps yield exactly $(e,u)$ and $(f,v)$. Under attack, $G\ket{D_j}$ denotes the honest value of the first register retained after the TTP undoes $\pi_E$.
      \item If the current data pad is $Q$, the corresponding key update in this warm-up is $Q_{\mathrm{new}}=GQG^\dagger$.
      \item This warm-up alone is not the final secure construction: without fresh Pauli re-encryption, correlations can persist between calls.
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Construction II: Add Re-encryption}
  ...
  \checkedgatediagram
    ...
    {$\ket{T_0}=\pi_0(\reenc{R_0}G X^e\ket{0^w},\,G X^u\ket{0^w})$}
    {$\ket{T_+}=\pi_+(\reenc{R_+}G Z^f\ket{+^w},\,G Z^v\ket{+^w})$}
    {check the trap outcomes $o_0,o_+$;\\[-0.1em]
      keep $\reenc{R_D}G\ket{D_j}$}

  \note{
    \scriptsize
    \begin{itemize}
      \setlength{\itemsep}{0pt}
      \item This is the same interaction as the previous frame. The coral symbols are the only additions: a fresh Pauli re-encryption and the corrections it induces.
      \item $R$ is a uniformly random Pauli on the permuted three-register block, sent to Bob as a classical description. Bob applies $G^{\otimes3}$ first and then $R$.
      \item Conjugating into the hidden register order gives $\pi_E^{-1}R\pi_E=R_D\otimes R_0\otimes R_+$. Bob knows $R$ in permuted coordinates but does not know which factor acts on data, $0$-traps, or $+$-traps.
      \item In the manuscript notation, $R$ is $P'_{\pi_E}$ and $(R_D,R_0,R_+)=(Q',P'_0,P'_+)$. The talk renames them so that $R$ reads as re-encryption.
      \item Since the directly supported gate normalizes Paulis, write $G^\dagger R_sG=X^{a_s}Z^{b_s}$. After applying $\pi_0^{-1}$ and $\pi_+^{-1}$, the expected strings are $(a_0\oplus e,u)$ and $(b_+\oplus f,v)$.
      \item On acceptance the TTP retains $R_DG\ket{D_j}$ and updates the data pad to $Q_{\mathrm{new}}=R_DGQG^\dagger$.
      \item A tensor product of three independent $w$-qubit Paulis is equivalently uniform in $\mathcal P_{3w}$; global Pauli phases are irrelevant.
...
\begin{frame}{Result: Feasibility with a Quantum TTP}
  ...
  \begin{itemize}
    ...
    \item $Q_0\,\mathsf{QEnc}(\ket0)$ and $Q_T\,\mathsf{QEnc}(\ket T)$ in the
      VQFHE evaluation key, where $\ket T:=T\ket+$;
  ...
  \vspace{0.65em}
  \pause
  {\normalsize \textbf{Online:} quantum storage and routing only; checks and
    key updates are classical.\par}
  ...
  \note{
    ...
    \begin{itemize}
      ...
      \item ``VQFHE evaluation key'' groups the encoded, padded $\ket0$ and $\ket T$ resources by role. A standard injection consumes $\ket T$ using an entangling gate, a measurement, and corrections.

...
\begin{frame}{Let Only One Direction Through}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.47\textwidth}
      \centering
      {\large If Alice rejects, fairness must restore $\qassetcopy{A}{A}$.\par}
    \column{0.47\textwidth}
      \centering
      {\large Bob accepts $\qassetcopy{A}{B}$.\par}
  \end{columns}
...
% -----------------------------------------------------------------------------
% 14--19. Six one-idea preliminaries
% -----------------------------------------------------------------------------
...
\begin{frame}{QEC: Spread One Qubit Across a Block}
  ...
  \begin{exampleblock}{Correct up to $\lambda$ damaged positions per block}
    ...
    $\displaystyle
      \mathsf{QDec}\!\left(
        \textcolor{QFECoral}{\mathcal A}\bigl(\mathsf{QEnc}(\rho)\bigr)
      \right)=\rho
      \quad\text{if $\mathcal A$ damages at most $\lambda$ positions.}$
  \end{exampleblock}

  \vspace{0.25em}
  {\large\bfseries\color{QFETeal}Bounded damage is repairable.\par}
...
\begin{frame}{Transversal Operations: Compute Coordinatewise}
  ...
  \vspace{0.55em}
  {\large\bfseries\color{QFETeal}
    Coordinatewise gates keep local damage local.\par}
...
\begin{frame}{Quantum Authentication: Detect Tampering}
  ...
  \begin{tikzpicture}[
    ...
    \node[anchor=east,align=right,font=\small] at (-3.35,0.78)
      {$\begin{aligned}
        ...
        \sigma&\leftarrow\mathsf{Auth}_k(\rho)
      \end{aligned}$};
    ...
    \draw[flow] (3.1,-0.48) -- (-3.1,-0.48)
      node[midway,above=2pt,font=\small,fill=white,inner sep=1pt]
      {returned state $\widetilde\sigma$};
    \node[anchor=east,font=\small] at (-3.35,-1.02)
      {$\mathsf{VerDec}_k(\widetilde\sigma)$};
    \draw[flow] (-3.1,-1.02) -- (-1.75,-1.02);
    \node[anchor=west,font=\large] at (-1.55,-1.02)
      {$\rejlabel\quad\text{or}\quad\rho_{\mathrm{out}}$};
  \end{tikzpicture}
  ...
  {\Large
    $\displaystyle
      ...
      \rho_{\mathrm{out}}\approx\rho.$\par}
...
\begin{frame}{The Trap Code: Hide Data Among Tests}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.56\textwidth}
      ...
      \centering
      {\color{QFEMuted}data $+$ $0$-traps $+$ $+$-traps,\\
        hidden by $\pi$ and a quantum one-time pad\par}
...
\begin{frame}{VQFHE: Compute, Then Verify}
  ...
  \begin{tikzpicture}[
    ...
    \node[anchor=east,font=\small] at (-3.35,-1.02)
      {$\mathsf{VerDec}_{\mathsf{sk}}(C,\widetilde{\mathsf{ct}},\mathsf{log})$};
    \draw[qmsg] (-3.1,-1.02) -- (-1.75,-1.02);
    \node[anchor=west,font=\large] at (-1.55,-1.02)
      {$\rejlabel\quad\text{or}\quad\ket y$};
  \end{tikzpicture}
  ...
  \note{
    \footnotesize
    \begin{itemize}
      \item This is the security interface, separated from the concrete $X$-gate mechanism on the next slide.
...
\begin{frame}{VQFHE by Example: $\mathsf{Eval}_X$}
  \centering
  {\small Ciphertext: encrypted state and pad keys.\quad
    Evaluation key: encrypted permutation.\par}
  ...
  {\Large
    $\displaystyle
      ...
      \qquad \widehat\pi\in\mathsf{evk}.$\par}
  ...
  \begin{align*}
    \widehat{s_\pi}
      &\leftarrow
        ...
        \bigl(\widehat\pi,\mathsf{HE.Enc}(1^n0^{2n})\bigr),\\[0.25em]
    \widehat x'
      &\leftarrow
        \mathsf{HE.Eval}_{\oplus}
        \bigl(\widehat x,\widehat{s_\pi}\bigr),\\[0.25em]
  ...
  \vspace{0.10em}
  {\large\bfseries\color{QFETeal}
    The quantum register is unchanged; the encrypted pad key is updated.\par}

  \vspace{0.30em}
  {\small Supporting every gate requires nontrivial gadgets
    \cite{alagic-et-al-vqfhe}.\par}

  \note{
    \footnotesize
    \begin{itemize}
      \item Hats denote classical homomorphic encryptions. The evaluation key supplies $\widehat\pi$; the evaluator learns neither the Pauli pad nor the secret trap permutation.
      \item The bit string $1^n0^{2n}$ marks the data block before permutation. Homomorphically applying the encrypted permutation produces an encryption of $s_\pi=\pi(1^n0^{2n})$.
      \item Updating $x$ to $x\oplus s_\pi$ makes the unchanged quantum register decrypt to $X\ket\psi$ while leaving both trap blocks unchanged.
      \item The log records the homomorphic permutation and XOR computations; it is not merely the gate label $X$.
      \item This is why $X$ is an intentionally easy example. The universal Alagic--Dulek--Schaffner--Speelman construction needs nontrivial gadgets and evaluation logs for the remaining gates.
    \end{itemize}
  }
\end{frame}

% -----------------------------------------------------------------------------
% 20--26. Candidate construction and resource boundary
% -----------------------------------------------------------------------------

\begin{frame}{Construction I: Teleport into Custody}
  \centering
  \begin{tikzpicture}[
      >={Latex[length=2.6mm]},
      qmsg/.style={->,very thick,draw=QFETeal},
    ...
    \node[font=\small,align=center,text width=3.05cm] at (3.25,1.18)
      {prepare EPR $(L_i,D_i)$\\encode, pad, and retain $D_i$};
    \draw[qmsg] (2.55,0.55) -- (-2.85,0.55)
      node[midway,above=2pt,font=\small,fill=white,inner sep=1pt]
      {EPR half $L_i$};
    \node[font=\small,align=center,text width=3.05cm] at (-3.55,-0.10)
  ...
  \note{
    ...
    \begin{itemize}
      ...
      \item This frame is only the construction. Save the interpretation of the final custody state for the next frame.
...
\begin{frame}{Construction I: State Now in Custody}
  \centering
  \vspace{0.15em}
  \begin{tikzpicture}
    \node[draw=QFEInk,fill=QFEInk,text=white,rounded corners=5pt,
      ...
      $\displaystyle
        \bigl(\widetilde\rho,Q^0\bigr),\qquad
        \widetilde\rho
  ...
  \note{
    ...
    \begin{itemize}
      ...
      \item No register carrying $\rho$ traveled Alice-to-TTP. Teleportation placed the state in the already encoded and padded retained EPR half.
...
\begin{frame}{Construction II: Evaluator Pass}
  \centering
  {\small For physical coordinate $j$, let $G:=g^{(j)}$ and let
    $\ket{D_j}$ be the padded data column.\par}

  \vspace{0.25em}
  {\small
    $\ket{E_0}=X^e\ket{0^w}$,\qquad
    $\ket{E_+}=Z^f\ket{+^w}$.
    The TTP samples $\pi_E\in S_3$ and fresh Paulis
    $Q',P'_0,P'_+$.\par}
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\scriptsize\bfseries,text=QFEMuted] at (3.65,1.05) {evaluator};
...
\begin{frame}{Construction II: Checker Pass}
  \centering
  {\small The TTP unshuffles Bob's return and keeps the transformed data
    $Q'G\ket{D_j}$. It prepares fresh known outputs\par}
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\scriptsize\bfseries,text=QFEMuted] at (-3.65,1.05) {checker};
    \node[font=\large\bfseries] at (3.65,1.35) {Quantum TTP};
  ...
  \vspace{0.15em}
  {\small The TTP accepts only the expected $Z/X$ outcomes. On acceptance,
    it records $Q_{\mathrm{new}}=Q'GQG^\dagger$.\par}
...
\begin{frame}{Construction III: Circuit Evaluation}
  ...
  {\small\bfseries\color{QFETeal}
    Each circuit step calls $\pi_{\mathsf{gate}}$ twice, with the roles reversed.\par}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.315\textwidth}
      \begin{exampleblock}{Checks}
        \centering
        fresh $0/+$ traps for gates\\
        fresh $0$-traps for measurements
...
\begin{frame}{Construction IV: Full Exchange}
  ...
  \note{
    ...
    \begin{itemize}
      ...
      \item This removes the copyable-classical-state step from the impossibility argument: the adjudicator now holds irreplaceable quantum custody.
